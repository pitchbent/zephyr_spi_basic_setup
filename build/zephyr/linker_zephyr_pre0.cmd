...
MEMORY
    ...
    ITCM : ORIGIN = 0, LENGTH = 65536 DTCM : ORIGIN = 536870912, LENGTH = 65536 OCRAM : ORIGIN = 538968064, LENGTH = 131072
 ...
 *(".kernel_noinit.*")
    ...
    _image_ram_end = .;
    _end = .;
    __kernel_ram_end = 0x80000000 + (32768 * 1K);
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    ITCM 0 (NOLOAD) : { __ITCM_start = .; KEEP(*(ITCM)) KEEP(*(ITCM.*)) __ITCM_end = .; } > ITCM __ITCM_size = __ITCM_end - __ITCM_start; __ITCM_load_start = LOADADDR(ITCM); DTCM 536870912 (NOLOAD) : { __DTCM_start = .; KEEP(*(DTCM)) KEEP(*(DTCM.*)) __DTCM_end = .; } > DTCM __DTCM_size = __DTCM_end - __DTCM_start; __DTCM_load_start = LOADADDR(DTCM); OCRAM 538968064 (NOLOAD) : { __OCRAM_start = .; KEEP(*(OCRAM)) KEEP(*(OCRAM.*)) __OCRAM_end = .; } > OCRAM __OCRAM_size = __OCRAM_end - __OCRAM_start; __OCRAM_load_start = LOADADDR(OCRAM);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
